    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            if self.levels > max_level:
                print(
                    f"Adjusting levels from {self.levels} to {max_level} to fit image size {H}x{W}."
                )
            ...
            bins = ((ang + 0.5 * bin_size) / bin_size).floor().to(
                torch.long
            ) % self.num_bins  # [B,C,H,W]
            ...
            for lvl in range(levels):
                ...
                hist_B = hist.view(B, C, num_cells, self.num_bins).reshape(
                    B, C * num_cells, self.num_bins
                )
