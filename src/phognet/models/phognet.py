...
class Conv2DModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size_main=3, kernel_size_skip=1, stride=1, padding_main=1, padding_skip=0):
        super(Conv2DModule, self).__init__()

        # Main branch
        self.conv1_main = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size_main, stride=stride, padding=padding_main)
        self.bn1_main = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU()

        self.conv2_main = nn.Conv2d(out_channels, out_channels, kernel_size=kernel_size_main, stride=1, padding=padding_main)
        ...
        if stride != 1 or in_channels != out_channels:
            self.shortcut1 = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size_skip, stride=stride, bias=False,padding=padding_skip),
...
class Conv1DModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size_main=3, kernel_size_skip=1, stride=1,stride_skip=1 ,padding_main=1, padding_skip=0):
        super(Conv1DModule, self).__init__()

        # Main branch
        self.conv1_main = nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size_main, padding=padding_main, stride=stride)
        self.bn1_main = nn.BatchNorm1d(out_channels)
        self.relu = nn.ReLU()

        self.conv2_main = nn.Conv1d(out_channels, out_channels, kernel_size=kernel_size_main, padding=padding_main, stride=stride)
        self.bn2_main = nn.BatchNorm1d(out_channels)

        # Skip/shortcut connection

        self.conv_skip = nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size_skip, stride=stride_skip, padding=padding_skip ,bias=False)
...
class PHOGProcessingBlock(nn.Module):
    def __init__(self, in_channels, out_channels, bins, levels, stride=1, ablation_case=None):
        ...
        """
        super(PHOGProcessingBlock, self).__init__()
...
class PHOGNetAblation(nn.Module):
    def __init__(self, block, num_blocks, num_classes, bins=20, levels=1, nInputPlane=1, ablation_case=None):
        """
        ...
        """
        super(PHOGNetAblation, self).__init__()
        self.bins = bins
        self.levels = levels
        ...
        # Stem Layer
        self.conv_stem = ConvBlock(nInputPlane, self.Num_OutputPlane[0], kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.layer1 = self._make_layer(block, self.Num_OutputPlane[0], num_blocks[0], stride=1, bins=self.bins, levels=self.levels, ablation_case=self.remove_phog_block)
        self.layer2 = self._make_layer(block, self.Num_OutputPlane[1], num_blocks[1], stride=2, bins=self.bins, levels=self.levels, ablation_case=self.remove_phog_block)
        self.layer3 = self._make_layer(block, self.Num_OutputPlane[2], num_blocks[2], stride=2, bins=self.bins, levels=self.levels, ablation_case=self.remove_phog_block)
        self.layer4 = self._make_layer(block, self.Num_OutputPlane[3], num_blocks[3], stride=2, bins=self.bins, levels=self.levels, ablation_case=self.remove_phog_block)
        ...
        if self.ablation_case == "remove_phog_block":
            ...
            multiplied_values = [self.Num_OutputPlane[i] * num_blocks[i] for i in range(len(self.Num_OutputPlane))]
    ...
    def _make_layer(self, block, out_channels, num_blocks, stride=1, bins=20, levels=1, ablation_case=None):
        layers = []
        ...
        layers.append(block(bins=bins, levels=levels, in_channels=self.in_channels, out_channels=out_channels, stride=stride,ablation_case=ablation_case))
        ...
        for _ in range(1, num_blocks):
            ...
            layers.append(block(bins=bins, levels=levels, in_channels=self.in_channels, out_channels=out_channels,ablation_case=ablation_case))
    ...
    def forward(self, x):
        ...
        if self.phog_layer is not None and self.conv1d_stem is not None:
            ...
            phog_out_stem = phog_out_stem.unsqueeze(2)  # Add channel dimension for Conv1D compatibility
        ...
        if all_phog_outputs:
            ...
            phog_concat = torch.zeros(x.size(0), 1).to(x.device)  # Default to a zero tensor if no PHOG output
...
class PHOGNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, bins=20, levels=1, nInputPlane=1):
        ...
        self.layer1 = self._make_layer(block, self.Num_OutputPlane[0], num_blocks[0], stride=1, bins=self.bins, levels=self.levels)
        self.layer2 = self._make_layer(block, self.Num_OutputPlane[1], num_blocks[1], stride=2, bins=self.bins, levels=self.levels)
        self.layer3 = self._make_layer(block, self.Num_OutputPlane[2], num_blocks[2], stride=2, bins=self.bins, levels=self.levels)
        self.layer4 = self._make_layer(block, self.Num_OutputPlane[3], num_blocks[3], stride=2, bins=self.bins, levels=self.levels)
    ...
    def _make_layer(self, block, out_channels, num_blocks, stride=1, bins=20, levels=1):
        ...
        # Add the first block with stride
        # print(f"Creating first block: in_channels={self.in_channels}, out_channels={out_channels}, stride={stride}")
        layers.append(block(bins=bins, levels=levels, in_channels=self.in_channels, out_channels=out_channels, stride=stride))
        ...
        for _ in range(1, num_blocks):
            #print(f"Creating additional block: in_channels={self.in_channels}, out_channels={out_channels}, stride=1")
            layers.append(block(bins=bins, levels=levels, in_channels=self.in_channels, out_channels=out_channels))
